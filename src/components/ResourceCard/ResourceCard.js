...
import {
  ...
  Error20,
  Launch20,
} from '@particle/icons-react';
import { settings } from 'particle-components';
...
export default class ResourceCard extends React.Component {
  render() {
    ...
    const particleTileclassNames = classnames([`${prefix}--tile`], {
    ...
    if (disabled === true || href === undefined) {
      cardContainer = <div className={particleTileclassNames}>{cardContent}</div>;
    } else if (isLink === true) {
      cardContainer = (
        <Link to={href} className={particleTileclassNames} {...rest}>
          {cardContent}
        </Link>
      );
    } else {
      ...
      cardContainer = (
        <a
          ...
          className={particleTileclassNames}
          {...rest}>
          {cardContent}
        </a>
      );
